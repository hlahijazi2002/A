...
const EmissionsAnalytics = () => {
  ...
  const { data: summaryRes, loading } = useFetch<any>(
    "/analytics/summary",
    null,
  );
      ...
      <div className="flex flex-wrap justify-between items-start gap-3">
        ...
        <div className="flex gap-2">
          <button className="flex items-center gap-2 px-3 py-2 border border-slate-200 rounded-lg text-[11px] font-bold text-slate-600">
            <Calendar size={13} className="text-slate-400" /> Jan – Dec 2026
          </button>
        </div>
      </div>
          ...
          {emissionsStats.map((stat, i) => (
            ...
            >
              ...
              <h2 className="text-2xl font-black text-slate-800 mb-3">
                {i === 0 && summary?.totalCo2eThisYear
                  ? `${(summary.totalCo2eThisYear / 1000).toFixed(1)}K`
                  : i === 1 && emissions?.data?.byScope?.SCOPE_1
                    ? `${(emissions.data.byScope.SCOPE_1 / 1000).toFixed(1)}K`
                    : i === 2 && emissions?.data?.byScope?.SCOPE_2
                      ? `${(emissions.data.byScope.SCOPE_2 / 1000).toFixed(1)}K`
                      : i === 3 && emissions?.data?.byScope?.SCOPE_3
                        ? `${(emissions.data.byScope.SCOPE_3 / 1000).toFixed(1)}K`
                        : stat.value}
              </h2>
              <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {emissionsTrend.map((item) => (
              ...
              >
                <div className="flex gap-1.5 items-end h-40 w-full justify-center">
                  <div
                    className="w-3 bg-teal-600 rounded-t-sm opacity-90"
          ...
          <div className="flex gap-5 mt-4 px-2">
            {SCOPE_LABELS.map((label, i) => (
              <div key={label} className="flex items-center gap-2">
                <div
                  className={`w-2.5 h-2.5 rounded-full ${SCOPE_COLORS[i]}`}
                />
        ...
        <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          <h3 className="font-bold text-sm text-slate-800 mb-6">
            Top Emitting Companies (2026)
          </h3>
          <div className="space-y-5">
            {topEmitters.map((co: any, i: number) => (
              <div key={i}>
                <div className="flex justify-between text-[11px] font-bold mb-1.5">
                  <span className="text-slate-700">{co.name || co.sector}</span>
                  <span className="text-slate-400">
                    {co.val || `${co.totalCo2e} tCO₂e`}
                  </span>
                </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between">
          ...
          <div className="mt-6 p-3.5 bg-teal-50/50 rounded-xl flex gap-3 items-start border border-teal-100/50">
            ...
            <p className="text-[10px] text-teal-800 leading-relaxed font-medium">
              ...
              {summary?.scope3Incomplete || 36} companies have incomplete Scope
              3 data.
            </p>
          </div>
        </div>
